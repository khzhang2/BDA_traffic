...
def main(rs, det):
    ...
    regr = make_pipeline(StandardScaler(),
                        LinearSVR(random_state=rs, tol=1e-7, max_iter=1e7))
...
pd.DataFrame(mean_data, columns=['nrmse', 'mape', 'smape', 'mae', 'nmae']).to_csv('./table_data/bda_SVR_mean.csv')
pd.DataFrame(std_data, columns=['nrmse', 'mape', 'smape', 'mae', 'nmae']).to_csv('./table_data/bda_SVR_std.csv')
